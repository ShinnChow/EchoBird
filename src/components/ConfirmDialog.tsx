    ...
    <ConfirmContext.Provider value={{ confirm }}>
      ...
      {isOpen && (
        ...
        >
          ...
          {/* Dialog box */}
          <div
            className={`relative w-[360px] max-w-[90vw] border bg-cyber-surface shadow-2xl rounded-xl overflow-hidden transition-all duration-200 ${
          ...
          >
            ...
            <div className="px-5 pt-4 pb-2 flex items-center gap-2">
              {(isDanger || isWarning) && (
                <AlertTriangle
                  className={`w-4 h-4 flex-shrink-0 ${isDanger ? 'text-red-400' : 'text-cyber-warning'}`}
                />
              )}
              {isInfo && (
                <svg
                  className="w-4 h-4 flex-shrink-0 text-cyber-text-secondary"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
