...
export function MotherAgentPanel() {
  ...
  const [sshTestResult, setSSHTestResult] = useState<{ success: boolean; message: string } | null>(
    null
  );
          ...
          >
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs text-cyber-text-secondary mb-1">
                    ...
                    <span className="opacity-50">({t('mother.optional')})</span>
                  </label>
                  <input
                    type="text"
            ...
            <div className="px-5 pb-4">
              ...
              {sshTestResult && !sshTesting && (
                <div
                  className={`text-[11px] font-mono mt-2 px-3 py-2 rounded border ${
              ...
              <button
                onClick={async () => {
                  ...
                  const existing = sshServers.find(
                    (s) => s.host === host && s.username === username
                  );
        ...
        {panelTab === 'servers' ? (
          ...
          <div className="space-y-1.5 text-[13px]">
            {[
                ...
                content: (
                  <>
                    ...
                    <div className="mt-2 text-xs space-y-0.5">
                      <p className="text-cyber-text-muted/70">
                        • <span className="text-cyber-text">{t('ssh.usernameHint')}</span> —{' '}
                        {t('ssh.cloudUsername')}
                      </p>
                      <p className="text-cyber-text-muted/70">
                        • <span className="text-cyber-text">{t('ssh.passwordHint')}</span> —{' '}
                        {t('ssh.cloudPassword')}
                      </p>
                      <p className="text-cyber-text-muted/70">
                        • <span className="text-cyber-text">{t('ssh.ipHint')}</span> —{' '}
                        {t('ssh.cloudIp')}
                      </p>
                      <p className="text-cyber-text-muted/70">
                        • <span className="text-cyber-text">{t('ssh.portHint')}</span> — 22
                      </p>
                    </div>
                  </>
                ),
              ...
              },
              {
                ...
                content: (
                  <>
                    ...
                    <p className="text-cyber-text-muted/50 mt-0.5">
                      {t('ssh.macOr')}{' '}
                      <span className="text-cyber-text">sudo systemsetup -setremotelogin on</span>
                    </p>
                ...
                content: (
                  <>
                    ...
                    <div className="mt-2 pt-1 border-t border-cyber-border/20 text-xs space-y-0.5">
                      <p className="text-cyber-text-muted/70">
                        • <span className="text-cyber-text">{t('ssh.usernameHint')}</span> —{' '}
                        {t('ssh.termuxUsername')}
                      </p>
                      <p className="text-cyber-text-muted/70">
                        • <span className="text-cyber-text">{t('ssh.passwordHint')}</span> —{' '}
                        {t('ssh.termuxPassword')}
                      </p>
                      <p className="text-cyber-text-muted/70">
                        • <span className="text-cyber-text">{t('ssh.ipHint')}</span> —{' '}
                        {t('ssh.termuxIp')}
                      ...
                      <p className="text-cyber-text-muted/70">
                        • <span className="text-cyber-text">{t('ssh.portHint')}</span> — 8022
                      </p>
                    </div>
                  </>
                ),
              ...
              <div
                key={section.id}
                className="border border-cyber-border/20 rounded overflow-hidden"
              >
                <button
                  onClick={() =>
                    setExpandedGuide((prev) => (prev === section.id ? null : section.id))
                  }
                ...
                >
                  ...
                  <ChevronDown
                    size={14}
                    className={`text-cyber-text/60 transition-transform ${expandedGuide === section.id ? 'rotate-180' : ''}`}
                  />
                </button>
                {expandedGuide === section.id && (
                  <div className="px-3 py-2 space-y-1">{section.content}</div>
                )}
              </div>
            ))}
          </div>
