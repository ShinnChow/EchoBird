//! Zhipu GLM usage provider
//!
//! API: GET https://open.bigmodel.cn/api/paas/v4/usage (or api.z.ai)
//! Response: { data: { limits: [{limit, used, nextResetTime}] } }
...
impl UsageProvider for ZhipuProvider {
    async fn query_usage(&self, api_key: &str, base_url: &str) -> Result<UsageResult, String> {
        ...
        // Detect CN or EN endpoint
        let url = if base_url.contains("bigmodel.cn") {
    ...
    fn can_handle(&self, base_url: &str) -> bool {
        base_url.contains("bigmodel.cn") || base_url.contains("api.z.ai")
    }
